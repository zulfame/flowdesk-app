...
async def purge_item(type: str, item_id: str, admin: dict = Depends(require_admin)):
    ...
    if not doc:
        raise HTTPException(status_code=404, detail="Data terhapus tidak ditemukan")
    # Hapus berkas fisik + metadata lampiran (termasuk parent komposit "<id>:<uid>",
    # dokumen tugas yang tersimpan via parent_id=taskId, dsb.)
    from services import purge_files
    await purge_files(item_id, prefix=True)
