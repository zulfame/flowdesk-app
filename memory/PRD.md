...
## Update (2026-06-08n) — Audit kepatuhan deploy Nexus Panel (akar masalah 502 di VPS)
**Akar masalah 502**: `backend/requirements.txt` masih hasil `pip freeze` pod Emergent — memuat `emergentintegrations==0.2.0` (butuh extra-index privat) dan `litellm @ https://customer-assets.emergentagent.com/...whl`, sehingga `pip install -r backend/requirements.txt` di panel GAGAL → backend tidak start → Nginx 502 saat login. Faktor kedua: `MONGO_URL`/`DB_NAME` tidak ada di `.env` VPS user, dan `db.py`/`security.py` memakai `os.environ[...]` sehingga crash tanpa pesan jelas.
Perubahan (minimal, tanpa mengubah fitur):
1. **`backend/requirements.txt` ditulis ulang** → 21 paket runtime yang benar-benar diimpor kode, semuanya dari PyPI publik (fastapi, uvicorn[standard], starlette, motor, pymongo, pydantic, email-validator, python-dotenv, python-multipart, PyJWT, bcrypt, cryptography, httpx, requests, boto3/botocore, openpyxl, pywebpush, py-vapid, http_ece). Diuji `pip install` + import di **venv bersih Python 3.11** → sukses. (`deploy/requirements.runtime.txt` tidak lagi diperlukan.)
2. **`backend/db.py`**: `MONGO_URL` → pesan `RuntimeError` yang jelas bila kosong; `DB_NAME` kini punya default `flowdesk`.
3. **`backend/security.py`** `_secret()`: `JWT_SECRET` kosong → pesan jelas (bukan `KeyError`); `routers/attachments.py` ikut memakai `_secret()`.
4. **`frontend/package.json`**: `build` → `DISABLE_ESLINT_PLUGIN=true CI=false craco build` agar `yarn build` tidak gagal karena warning ESLint (diuji: build sukses).
5. **Artefak Docker kustom dihapus**: seluruh folder `deploy/` (docker-compose.yml, Dockerfile backend & frontend, start/stop/seed/deploy.sh) — panel yang menghasilkan Docker sendiri.
6. **`backend/.env.example` & `frontend/.env.example` ditambahkan**, `.gitignore` diberi pengecualian `!.env.example` / `!*/.env.example` agar contoh ikut ter-push.
7. **README**: bagian env ganda dirapikan → hanya satu `## Environment Variables` (tabel 4 kolom) + baris baru `APP_ENCRYPTION_KEY`, `DB_NAME` menjadi Optional (default `flowdesk`); bagian "Deploy Lokal dengan Docker" dihapus, struktur proyek & Daftar Isi diperbarui.
Verifikasi: `server:app` = FastAPI, **103 rute API, 0 rute tanpa prefiks `/api`**, tidak ada URL/port/secret hardcoded (grep bersih), penyimpanan file tetap di `LOCAL_STORAGE_DIR` (default `/app/data`), seed admin idempoten dari `ADMIN_EMAIL`/`ADMIN_PASSWORD`, login lokal 200 setelah restart.

## Update (2026-06-08o) — Perbaikan crash boot di VPS (502 lanjutan)
Log VPS menunjukkan crash kedua: `crypto.py` → `ValueError: Fernet key must be 32 url-safe base64-encoded bytes` karena `APP_ENCRYPTION_KEY` user berupa **hex 96 karakter**, bukan kunci Fernet. Perbaikan agar boot tidak pernah gagal karena env:
1. **`crypto.py`**: `_build_fernet()` — coba `Fernet(key)`; bila tidak valid, kunci **diturunkan otomatis** `base64url(sha256(key))` + log peringatan berisi cara membuat kunci baku. Tidak crash lagi.
2. **`db.py`**: `MONGO_URL` kosong → **default `mongodb://localhost:27017`** + log peringatan (tidak lagi `RuntimeError`); nilai dibersihkan dari kutip (`"mongodb://..."` tetap valid); `serverSelectionTimeoutMS` dari `MONGO_TIMEOUT_MS` (default 5000) agar tidak menggantung.
3. **`server.py`**: startup tidak lagi memblokir — bootstrap (indeks, seed admin, sinkron peran, storage, loop) dipindah ke `_bootstrap_with_retry()` sebagai **background task** dengan 20× retry @3s. Efek: app langsung melayani `/api/` (200) walau Mongo belum siap, dan seed otomatis jalan begitu Mongo hidup.
4. **`routers/database.py`**: `BACKUP_DIR.mkdir()` dipindah dari import-time ke `_ensure_backup_dir()` (dipanggil saat backup dibuat) agar import tidak gagal bila volume belum siap.
5. **README**: `MONGO_URL` jadi Optional (default localhost) + baris baru `MONGO_TIMEOUT_MS`, deskripsi `APP_ENCRYPTION_KEY` diperbarui (kunci bebas diterima).
Verifikasi: uvicorn dijalankan di **venv bersih Python 3.11** dengan env PERSIS milik user (key hex, MONGO_URL berkutip, ADMIN_EMAIL/PASSWORD user) → import sukses, `Bootstrap selesai`, **login `sa@bprbangunarta.co.id` 200**. Skenario Mongo mati → `/api/` tetap 200 (bukan 502), login 500 dengan retry bootstrap berjalan. Backend pod restart & login admin lokal 200. DB uji `flowdesk_uji` dihapus.

## Update (2026-06-08p) — Open Graph benar-benar terpakai saat tautan dibagikan + Kontak & Footer di halaman Login
**Akar masalah pratinjau tautan "Emergent | Fullstack App"**: (1) `frontend/public/index.html` masih template Emergent (title "Emergent | Fullstack App", description "A product of emergent.sh", skrip emergent-main.js & PostHog) dan crawler membaca file statis itu, bukan `/api/og/render`; (2) gambar OG hasil unggahan tersimpan sebagai **data URI base64** di DB, dan crawler (WhatsApp/Telegram/FB) TIDAK bisa memakai `og:image` berupa data URI.
Perbaikan:
1. **`frontend/public/index.html` ditulis ulang**: judul & deskripsi FlowDesk (bukan Emergent), meta OG/Twitter lengkap, skrip analitik pihak ketiga (PostHog & emergent-main.js) dihapus, dan `og:image`/`twitter:image` menunjuk `%REACT_APP_BACKEND_URL%/api/og/image` sehingga **gambar OG selalu mengikuti Kelola Aplikasi tanpa build ulang**.
2. **`routers/og.py`**: endpoint baru `GET /api/og/image` — men-decode data URI menjadi **berkas gambar nyata** (`Content-Type` asli, cache 5 menit) atau redirect bila og_image berupa URL http. `_build_html()` kini memakai URL absolut `<base>/api/og/image` untuk `og:image` (+ `og:image:width/height` 1200×630), dan base URL diambil dari `canonical_url`/`app_url` atau header `X-Forwarded-Proto/Host` (aman di belakang reverse proxy).
3. **Dokumentasi & UI**: kartu Pratinjau Tautan di Kelola Aplikasi menampilkan **snippet Nginx** (map user-agent crawler → `proxy_pass /api/og/render`) agar judul & deskripsi juga dinamis; README menambah bagian **"Pratinjau Tautan (Open Graph)"** dengan snippet yang sama + Daftar Isi diperbarui.
4. **Kontak & Footer diterapkan di halaman Login** (`components/layout/AuthLayout.jsx`): `support_email` tampil sebagai tautan "Butuh bantuan? <email>" (mailto) dan `footer_text` dipakai sebagai teks hak cipta — di bawah kartu login (`data-testid="auth-footer"`, `auth-support-email`, `auth-footer-text`) sekaligus di panel merek kiri (`auth-brand-copyright`, fallback `© <tahun> <app_name>`).
Verifikasi: `GET /api/og/render` → `og:image` kini URL http absolut; `GET /api/og/image` → **200 image/jpeg 39 KB**; halaman login menampilkan "Butuh bantuan? info@bprbangunarta.co.id" + "© 2026 BPR Bangunarta. All Rights Reserved." (screenshot). `design-guard.sh` exit 0.
Catatan: judul/deskripsi OG hanya ikut dinamis bila snippet Nginx dipasang; tanpa itu dipakai nilai statis di `index.html` (bukan lagi Emergent).

## Update (2026-06-08q) — Tiket Bantuan: celah manipulasi ditutup, Penanganan diperkaya, komentar berlampiran + anti-duplikasi
1. **Celah manipulasi ditutup (P0)**: penerima tiket **tidak lagi** boleh mengubah isi tiket. `PUT /api/help-tickets/{id}` → `title/description/category/priority/attachments` **hanya pelapor atau Super Admin** (403 "Hanya pelapor tiket yang dapat mengubah isi tiket"); `resolution` & `resolution_attachments` **hanya penerima** (403 untuk pelapor); `can_edit` kini `owner || admin` saja (sebelumnya termasuk penerima) sehingga ikon sunting pada kartu ringkasan, Informasi Tiket, dan Lampiran hilang bagi penerima. Aturan pengalihan tujuan (pelapor / atasan penerima) tidak berubah.
2. **Kartu "Penanganan" dipindah ke kolom kiri, di ATAS kartu "Komentar"** dan diperkaya: Status, Penerima, Diselesaikan, Catatan Penyelesaian, dan **Bukti Pengerjaan** (lampiran multi via `DocumentManager`, field baru `resolution_attachments`, tombol URL + Unggah, hanya untuk penerima). Mode baca menampilkan chip lampiran; ada keterangan bila pengguna bukan penerima.
